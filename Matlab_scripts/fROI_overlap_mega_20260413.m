...
if (strcmp(expt, 'events2move_instrsep') && ismember(string(uid), ["408","776","775"])) || (strcmp(expt, 'EventsRev_instrsep') && ismember(string(uid), ["770", "773", "774", "775"])) || 
  (strcmp(expt, 'spatialFIN') && ismember(string(uid), ["419"])
    fROI_path = fullfile(data_dir, session, 'DefaultMNI_PlusStructural', 'results', 'firstlevel', expt, get_fROI_name(contrast_nums, parcel_hashname));
else
    fROI_path = fullfile(data_dir, session, ['firstlevel_' expt], get_fROI_name(contrast_nums, parcel_hashname));
end
