...
read_data <- function(prefix, loctasks) {
  base_dir = paste('../../TripleEvents_data/data/results_mROI/', prefix, sep='')
...
i = 0;
for (prefix in c('left_ATL_inferior_pole', 'right_ATL_inferior_pole', 'left_ATL_inferior_pole_middle', 'right_ATL_inferior_pole_middle')) {
  if (!is.null(dat)) {
    dat0 = read_data(prefix, event_expts)
    dat0$ROI = as.numeric(dat0$ROI) + i
    i = i + 1
    dat = rbind(dat, dat0)
  } else {
    dat = read_data(prefix, event_expts)
    dat$ROI = as.numeric(dat$ROI)
    i = i + 1
  }
}
...
#                    '2'='F2', '4'='F1', '14'='TP3', '12'='TP2', '20'='Cer1', #'10'='TP4',
#                    '6'='F3', '8'='TP1', '16'='O1', '22'='Cer2', '18'='O2'))
#dat$ROI = factor(dat$ROI, #levels=c('F1','F2','F3','TP1','TP2','TP3','TP4','Cer1','Cer2','O1','O2'))

dat = dat %>%
  mutate(ROI=recode(ROI, 
                    '1'='LInfPole', '2'='RInfPole', '3'='LInfPoleMid', '4'='RInfPoleMid'))
dat$ROI = factor(dat$ROI, levels=c('LInfPole', 'RInfPole', 'LInfPoleMid', 'RInfPoleMid'))

# EDITTED Remove rows where ROI is NA (i.e., levels not in the specified list)
#dat <- dat[!is.na(dat$ROI), ]
#
#dat = dat %>% 
...
plot_fROI_responses(dat, split_by_ROIgroup = FALSE, ylim=c(-1,1))
ggsave(paste(plot_dir, prefix, plot_name,' by ROI group_rescaled.png',sep=''), 
...
plt.by_expt.mean = plot_fROI_responses(dat.mean, split_by_expt=TRUE, show_legend=FALSE, show_facet_labels=FALSE, ylim=c(-.5,.5))
plt.by_expt.ROI = plot_fROI_responses(dat %>% filter(Effect %in% c('Sent_Sem', 'Sent_Perc', 'Pic_Sem', 'Pic_Perc')), split_by_expt=TRUE, ylim=c(-.5,.5))

plt.by_expt.mean + plt.by_expt.ROI + plot_layout(widths=c(1,6))
ggsave(paste(plot_dir, prefix, plot_name,' by expt_rescaled.png',sep=''), 
