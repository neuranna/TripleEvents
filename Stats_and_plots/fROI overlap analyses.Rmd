...
dat = dat %>% 
  filter(!(ROI1 %in% c("O1", "O2"))) %>% 
  filter(!(ROI2 %in% c("O1", "O2")))

d = dat %>% 
  filter(Network2=='events') %>%
  filter(Network1=='language') %>%
...
ggplot(d)+
  geom_tile(mapping=aes(x=ROI1, y=ROI2, fill=meanOverlap), color="black")+
...
ggsave(paste(plot_dir, "overlap_heatmap_lang.png", sep=""),
      width=plot_width*0.5, height=8, units='cm')
```
...
ggsave(paste(plot_dir, "overlap_heatmap_MD_DMN.svg", sep=""),
      width=plot_width*0.5, height=8, units='cm')


# SMALLER PLOT
#keep only nonzero overlap ROIs
d.totaloverlap = d %>% group_by(ROI1) %>%
  summarize(overlapsum = sum(meanOverlap)) %>% 
  filter(overlapsum>0.01)  # threshold here

d.short = d %>%
  filter(ROI1 %in% d.totaloverlap$ROI1) %>%
  mutate(ROI1 = paste(Network1, ROI1))
d.short$ROI1 = fct_relevel(d.short$ROI1, "DMN TPJ", after = Inf)
...
ggsave(paste(plot_dir, "overlap_heatmap_MD_DMN_short.svg", sep=""),
      width=plot_width*0.5, height=8, units='cm')
```
