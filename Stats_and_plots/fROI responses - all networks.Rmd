...
read_data <- function(prefix, loctasks, add_objectsem=FALSE) {
  base_dir = paste('../data/results_fROI_responses/', prefix, sep='')
...
set_contrasts <- function(dat.stats) {
  ...
  h = hypr(S~N, H~E, 0~0.5*(Sent_Sem+Pic_Sem), 0.5*(Sent_Sem+Pic_Sem)~0.5*(Sent_Perc+Pic_Perc), (Sent_Sem+Sent_Perc)~(Pic_Sem+Pic_Perc), Sent_Sem-Sent_Perc~Pic_Sem-Pic_Perc,    # main
...
```{r}
default_colors = c('#B20000', '#FF9999', '#0A730A', '#98CD92', 
...
plot_fROI_responses <- function (df, split_by_expt=FALSE, split_by_ROIgroup=FALSE, 
                                 ...
                                 plot_title=NULL,plot_subjectdata=TRUE, ylabel='Effect size'){
...
f = ggplot(data=df)+
  ...
  labs(x=NULL, y=ylabel, title=plot_title)+
...
model = lmer(EffectSize ~ Effect*Experiment + (1|ROI) + (1|Subject), data=dat.stats, REML=FALSE)
...
contrasts(dat.stats.byexpt$StimType) <- contr.sum(levels(dat.stats.byexpt$StimType))
dat.stats.byexpt$Task = factor(dat.stats.byexpt$Task, levels=c('Sem', 'Perc'))
...
```{r}
# Contrast effects in each experiment, FDR-corrected across expts

model.dfs <- list()
index = 1

for (expt in unique(dat.stats.byexpt$Experiment)) {
    dat.expt = dat.stats.byexpt %>% filter(Experiment==expt) 
    model = lmer(EffectSize ~ Task*StimType + (1|ROI) + (1+Task+StimType|Subject), data=dat.expt, REML=FALSE,
                 control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
    ...
    names(model.df) = c('Regression Term', 'Beta', 'SE', 'p.value', 'Experiment')
    model.df = model.df[,c(5,1,2,3,4)] %>%
      mutate(Beta = round(Beta, 2), 
             SE = round(SE, 2),
             p.value = ifelse(p.value>0.001, round(p.value, 3),
...
write.table(stats_df, file = "Tables/semantic stats by experiment.csv", sep = ",", quote = FALSE, row.names = F)
```
...
### By ROI group
```{r}
plot_name = 'EventsAll occipital'
plot_fROI_responses(dat.occipital, split_by_ROIgroup = TRUE, ylim=c(-4.6,10), col_width=0.75)+theme(legend.position = "right")

ggsave(paste(plot_dir, plot_name,'.png',sep=''), 
       width=ROI_plot_width-8, height=plot_height_1row+3, units='cm')
```
...
plt.by_expt.ROI = plot_fROI_responses(dat %>% filter(Effect %in% c('Sent_Sem', 'Sent_Perc', 'Pic_Sem', 'Pic_Perc')), split_by_expt=TRUE)+theme(legend.position = "bottom")
...
#### Sample fROI (F2)
```{r}
plot_name = 'Events Parcels F2 by expt'
...
plt.by_expt.mean = plot_fROI_responses(dat %>% filter(ROI=='F2') %>%
  filter(Effect %in% c('Sent_Sem', 'Sent_Perc', 'Pic_Sem', 'Pic_Perc')), 
...
dat.mean = dat %>%
  filter(Effect %in% c('Sent_Sem', 'Sent_Perc', 'Pic_Sem', 'Pic_Perc', 'S', 'N')) %>%
  group_by(Experiment, Subject, Effect) %>%
  summarize(EffectSize = mean(EffectSize)) %>%
  mutate(ROI = "Average")

plt.by_expt.mean = plot_fROI_responses(dat.mean, 
  split_by_expt=FALSE, show_legend=FALSE, show_facet_labels=FALSE)

ggsave(paste(plot_dir, plot_name,' across all expt SN.png',sep=''), 
       width=hemi_plot_width-10, height=plot_height_1row+5, units='cm')
```
...
```{r}
plot_name = 'Events Parcels Avg'
colors = c('#B20000', '#FF9999', '#0A730A', '#98CD92', 
                             '#0000B2', '#B2B2FF')

dat.mean = dat %>%
  filter(Effect %in% c('Sent_Sem', 'Sent_Perc', 'Pic_Sem', 'Pic_Perc', 'H', 'E')) %>%
  group_by(Experiment, Subject, Effect) %>%
  summarize(EffectSize = mean(EffectSize)) %>%
  mutate(ROI = "Average")

plt.by_expt.mean = plot_fROI_responses(dat.mean, 
  split_by_expt=FALSE, show_legend=FALSE, show_facet_labels=FALSE, colors=colors)

ggsave(paste(plot_dir, plot_name,' across all expt HE.png',sep=''), 
       width=hemi_plot_width-10, height=plot_height_1row+5, units='cm')
```
...
# FDR correction
stats_df = correct_FDR(model.df)

write.table(stats_df, file = "Tables/Lang stats.csv", sep = ",", quote = FALSE, row.names = F)
```
...
Do not plot MD responses

```{r}
dat = dat %>% filter(!(Effect %in% c('E', 'H')))
```
...
p.lang_avg = plot_fROI_responses(dat.mean, show_legend = FALSE)
p.lang_avg

ggsave(paste(plot_dir, plot_name,' avg.png',sep=''), 
       width=hemi_plot_width-10, height=plot_height_1row, units='cm')
```
...
```{r}
plot_name = 'Language Network - core'
p.lang_roi = plot_fROI_responses(dat, ylabel="")
p.lang_roi
ggsave(paste('plots/',plot_name,'_ROI.png',sep=''), 
       width=ROI_plot_width, height=plot_height_1row+4, units='cm')
```

### Combine

```{r}
p.lang_avg + p.lang_roi + plot_layout(widths = c(1,3))
ggsave(paste('plots/',plot_name,'_avg_and_ROI.png',sep=''), 
       width=ROI_plot_width, height=plot_height_1row+4, units='cm')
```
...
dat.mean <- dat %>% 
  filter(!(Effect %in% c("S", "N"))) %>%
  group_by(Effect, Subject, Experiment, Hemisphere) %>% 
  summarize(EffectSize = mean(EffectSize)) %>%
  mutate(ROI=Hemisphere)

plot_title = 'Multiple Demand Network'
p.MD = plot_fROI_responses(dat.mean, plot_title = plot_title, show_legend = FALSE, colors=colors)
p.MD 

ggsave(paste('plots/',plot_title,'_hemi.png',sep=''),
       width=hemi_plot_width, height=plot_height_1row, units='cm')
```
...
p.DMN = plot_fROI_responses(dat.mean, colors=colors, plot_title = plot_title)
p.DMN
```
...
# COMBINE PLOTS 2 NETWORKS
```{r}
p.MD + p.DMN
ggsave(paste('Plots/', 'twonetworks by hemi.png', sep=''), 
       width=ROI_plot_width, height=plot_height_1row+10, units='cm')
```
...
dat.mean <- dat %>% 
  ...
  summarize(EffectSize = mean(EffectSize)) %>%
  filter(!(Effect %in% c('H', 'E')))

plot_title = 'vATL'
p.vatl = plot_fROI_responses(dat.mean, show_legend = FALSE, ylim=c(-1, 1))
p.vatl

ggsave(paste(plot_dir, plot_title,' SN.png',sep=''), 
       width=hemi_plot_width-5, height=plot_height_1row+5, units='cm')
```
...
p.RHsem = plot_fROI_responses(dat %>% filter(Effect %in% c('Sent_Sem', 'Sent_Perc', 'Pic_Sem', 'Pic_Perc')), split_by_ROIgroup = FALSE, ylim=c(-1,3.5), col_width=0.82, plot_title = plot_title) + theme(legend.position="bottom")

p.RHsem

ggsave(paste('Plots/', plot_title, '.png', sep=''), 
