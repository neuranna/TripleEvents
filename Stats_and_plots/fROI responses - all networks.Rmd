...
m = lmer(EffectSize ~ Effect*Experiment*Hemisphere + (1|Subject) + (1|ROI), data=dat.stats, REML=FALSE)
summary(m)
```

As expected, response to sentence reading was stronger than to nonword reading ($\beta$=`r round(coef(summary(m))[2,1], 2)`, SE=`r round(coef(summary(m))[2,2], 2)`, p=`r round(coef(summary(m))[2,5], 3)`).
For the critical experiments, there was a strong effect of task (semantic > perceptual; $\beta$=`r round(coef(summary(m))[4,1], 2)`, SE=`r round(coef(summary(m))[4,2], 2)`, p=`r round(coef(summary(m))[4,5], 3)`), 
stimulus type (sentences > pictures; $\beta$=`r round(coef(summary(m))[5,1], 2)`, SE=`r round(coef(summary(m))[5,2], 2)`, p=`r round(coef(summary(m))[5,5], 3)`), 
and an interaction between task type and stimulus type ($\beta$=`r round(coef(summary(m))[6,1], 2)`, SE=`r round(coef(summary(m))[6,2], 2)`, p=`r round(coef(summary(m))[6,5], 3)`), 
indicating a preference for sentences during the semantic task. These results are consistent with those reported in Ivanova et al (2021), who analyzed a subset of these data (specifically, Experiment 3).

The sentence reading>nonword reading was stronger in left hemisphere ($\beta$=`r round(coef(summary(m))['EffectS-N:HemisphereRH',1], 2)`, SE=`r round(coef(summary(m))['EffectS-N:HemisphereRH',2], 2)`, p=`r round(coef(summary(m))['EffectS-N:HemisphereRH',5], 3)`), in line with numerous accounts of left-lateralized responses to language. For the critical experiments, the effects of semantic 
There was no significant difference between hemispheres in response strength to semantic tasks (collapsed across stimulus modalities) with respect to the baseline (p=.060). 
...
write.table(stats_df, file = "Tables/language ROI stats.csv", sep = ",", quote = FALSE, row.names = F)
```
